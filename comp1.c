...
/*                                                                          */
/*                                                                          */
/*       Skeleton for the comp1.c compiler.  This compiler should be able   */
/*       to compile CPL programs that don't contain procedures.  It         */
/*       should output a listing file (like the parsers) and also an        */
...
/*       the Qt build system.                                               */
/*                                                                          */
/*       Implementation of comp1 is       									*/
/*       a copy your tested and working version of parser2.c and used it    */
/*       as a base to build comp1.                                          */
/*                                                                          */
/*                                                                          */
/*     															            */
/*                                                                          */
...
int scope = 0; //Declaring scope variable
...
PRIVATE void ParseRestOfStatement( SYMBOL *target )
{
	...
	if (CurrentToken.code == LEFTPARENTHESIS){
		...
		if ( target != NULL )//nested if else statements
			if ( target->class == CLASS_PROCEDURE )
				Emit( I_CALL, target->address );
			else 
			{
				sprintf(errorbuffer, "\"%s\" is not a procedure\n", target ->s);//error buffering
	...
	else if (CurrentToken.code == ASSIGNMENT) 
	{
		...
		if ( target != NULL )//nested if else statements
			if ( target->class == CLASS_VARIABLE ){
				...
				sprintf(errorbuffer, "\"%s\" is not a global variable\n", target ->s);
				Error( errorbuffer, CurrentToken.pos );
			}
	}
}
...
PRIVATE int ParseRelOp( void ){
	...
	switch(CurrentToken.code)//Changed to switch statement from parser2.c for ease of use
    {
		 case GREATER : 
			 instruction = I_BLZ;
			 Accept( GREATER ); 
...
PRIVATE void SetupSets( void ){
	...
	//PRIMARY Used for <ProcDeclaration>
...
PRIVATE SYMBOL *LookupSymbol(void){ // Look Up Symbol method
	static char errorbuffer[M_LINE_WIDTH+2];
	SYMBOL *sptr;
